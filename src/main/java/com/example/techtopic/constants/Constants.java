package com.example.techtopic.constants;

public class Constants {
    public static final String DATA_STORAGE_MAP = "data_storage_map";
    public static final String GET_COMMAND = "get ";
    public static final String SET_COMMAND = "set ";
    public static final String LOAD_COMMAND = "load ";
    public static final String SAVE_COMMAND = "save ";
    public static final String REVERSE_COMMAND = "reverse ";
    public static final String COUNT_WORDS_COMMAND = "count-words ";

    public static final String EXIT_COMMAND = "exit";
    public static final String OUTPUT_DELIMITER = "=";
    public static final String INITIAL_MASSAGE = "Enter data, please:";
    public static final String OUTPUT_ERROR_MASSAGE = "Err: no value for ";
    public static final String OUTPUT_SET_MASSAGE = "Saved ";
    public static final String OUTPUT_LOAD_MASSAGE = "Data from  is loaded";
    public static final String OUTPUT_SAVE_MASSAGE = "Data exported to ";
    public static final String OUTPUT_WORD_COUNT_MASSAGE = "Words in";
    public static final String OUTPUT_WRONG_COMMAND_MASSAGE = "Enter valid command, please";
    public static final String FILE_ERROR_MASSAGE = "File not exist";

}
